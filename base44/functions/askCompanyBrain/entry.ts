...
Deno.serve(async (req) => {
  try {
    ...
    // 利用制限チェック（Gemini API呼び出し前）
    const PLAN_LIMITS_CHECK = {
      Light: { aiAnswerLimitMonthly: 1000 },
      Standard: { aiAnswerLimitMonthly: 5000 },
      Professional: { aiAnswerLimitMonthly: 20000 },
      Enterprise: { aiAnswerLimitMonthly: null },
    };

    const limitConfig = PLAN_LIMITS_CHECK[planName] || PLAN_LIMITS_CHECK.Light;
    const aiAnswerLimit = limitConfig.aiAnswerLimitMonthly;

    if (aiAnswerLimit !== null && monthlyConversations.length >= aiAnswerLimit) {
      return json({
        answer: "月間AI回答数の上限に達しました。プラン変更または追加利用をご検討ください。",
        confidence: 0,
        needHumanReview: true,
...
`.trim();
    ...
    if (!geminiRes.ok) {
      ...
      return json({ error: "Gemini API error", status: geminiRes.status, detail }, 500);
    }
