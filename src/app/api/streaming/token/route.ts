...
- 説明の長さは質問に合わせてよい。ただし話し終わりは必ず文として完結させる
...
- ユーザーの言い淀みや言い直しは無視して、最後に言いたかった内容を拾う`;
  ...
  const liveConfig = {
    ...
    // Explicitly raise the per-turn output ceiling. With manual turn
    // control the remaining truncations only hit very long answers
    // ("構造耐力・防火・避難をそれぞれ詳しく"), which points at the
    // turn's default output token limit. Bump it high so a long,
    // multi-topic spoken answer can finish in one turn.
