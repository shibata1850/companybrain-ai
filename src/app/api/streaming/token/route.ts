...
export async function POST(req: NextRequest) {
  ...
  // Voice availability is signalled separately (textOnly below): the
  // client then skips the Live connection and asks over plain HTTP.
  ...
  // 音声が使えないユーザーには Live 接続自体を行わせない。ネイティブ
  // 音声モデルは TEXT モダリティを受け付けない(code 1007 で切断)
  // ため、テキスト質問は通常のテキストAPI(/api/avatars/[id]/ask)で
  // 処理する。クライアントはこのフラグを見て Live 接続をスキップする。
  if (!voiceEnabled) {
    return NextResponse.json({
      textOnly: true,
      voiceEnabled: false,
